...
window.showAsset=id=>{const a=scenario.assets.find(x=>x.id===id); $('drawerContent').innerHTML=`<div class="eyebrow">DIGITAL TWIN ASSET PROFILE</div><h2>${a?.name||'Station node'}</h2><p>Fictional demonstration corridor object. Synthetic data is clearly separated from production adapters.</p>`; $('drawer').classList.add('open')}
...
async function loadRealMap(){
	...
	if(!response.ok){$('eventStatus').textContent='Public map unavailable; synthetic twin remains active.'; return}
...
async function openSearchResult(kind,id){
	...
	$('drawerContent').innerHTML=`<div class="eyebrow">${kind.toUpperCase()} RECORD</div><h2>${record.id} · ${result.label}</h2><p>${result.detail}</p><div class="factor"><span>Section</span><strong>${record.section||'Network'}</strong></div><div class="factor"><span>Source state</span><strong>${scenario.synthetic?'SYNTHETIC DEMO':'PUBLIC DATA'}</strong></div>`;
...
async function renderModuleView(name){
	...
	else if(name==='Corridor Digital Twin') html='<p>Inspect the synthetic corridor and load public OpenStreetMap geometry.</p><button class="button secondary" id="moduleMap">Load real map</button><div class="module-grid" id="moduleOutput"></div>';
	else if(name==='Maintenance Backlog') html=`<div class="module-table">${scenario.tasks.map(task=>`<div><strong>${task.id}</strong><span>${task.department} · ${task.description}</span><b>${task.priority} / ${task.priority_score}</b></div>`).join('')}</div>`;
	...
	else if(name==='Models'){const data=await get('/models'); html=`<p>${data.models.length} active models. Synthetic models are marked; imported models show source provenance.</p><div class="module-table">${data.models.map(model=>`<div><strong>${model.name}</strong><span>${model.algorithm} · ${model.dataset}</span><b>${model.version}</b></div>`).join('')}</div><button class="button primary" id="moduleTrainModels">Retrain models</button>`}
